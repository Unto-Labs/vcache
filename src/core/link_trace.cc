...
bool IsIgnoredPath(const std::string& path) {
  ...
  if (HasPrefix(path, "/dev/")) return true;
  if (HasPrefix(path, "/run/")) return true;
  ...
  // Locale and character-set data. Read by almost every tool for message
  // formatting; affects diagnostics, not the linked bytes.
  if (path.find("/locale") != std::string::npos) return true;
  if (path.find("/gconv/") != std::string::npos) return true;
  if (util::EndsWith(path, "/gconv-modules")) return true;
...
bool ParseTraceLog(const std::string& log_path,
  ...
  for (const std::string& line : util::Split(*text, '\n', /*skip_empty=*/true)) {
    if (line.size() < 3 || line[1] != '\t') continue;  // malformed; skip
    const char kind = line[0];
    std::string path = line.substr(2);
    if (path.empty()) continue;
    if (path[0] != '/') path = util::AbsoluteLexical(cwd + "/" + path);
    ...
    switch (kind) {
      ...
      default: break;  // 'D' listings are covered by the absent set
    }
  }
